...
fun VersionCatalog.updateFrom(
    ...
    val libraries =
        this.libraries.toMutableMap().apply {
    ...
    val plugins =
        this.plugins.toMutableMap().apply {
...
private fun VersionCatalog.retainCurrentVersionReferences(
    ...
    val versionsToCurrentKeys =
        this.versions.mapValues { version ->
 ...
 */
fun VersionCatalog.mapPlugins(plugins: Map<String, String>): VersionCatalog {
    if (plugins.isEmpty()) {
        return this
    }
...
private fun VersionCatalog.collectVersionReferenceForGroups(
    ...
    val resolvedVersions =
        librariesByGroup.mapValues { entry ->
            entry.value.map {
                it.copy(version = it.resolvedVersion(copy(versions = versions)))
            }
        }
    val librariesWithoutVersionRef =
        libraries.filterValues {
    ...
    for (library in librariesWithoutVersionRef) {
        ...
        val groupVersions =
            group
                .asSequence()
                .mapNotNull { (it.version as? VersionDefinition.Simple)?.version }
                .groupBy { it }
                .filter {
                    it.value.size > 1
                }.map {
        ...
        if ((library.value.version as? VersionDefinition.Simple)?.version == groupVersions.first()) {
            val versionRef =
                librariesByGroup[library.value.group]
                    ?.firstOrNull {
...
internal fun VersionCatalog.pruneVersions(): VersionCatalog {
    ...
    val versions =
        this.versions.toMutableMap().apply {
