...
private fun Map<String, Any>.toPluginDependencyMap(): Map<String, Plugin> =
    toTypedMap<Any>().mapValues { entry ->
...
private fun Map<String, Any>.toDependencyMap(): Map<String, Library> =
    toTypedMap<Any>().mapValues { entry ->
        when (val value = entry.value) {
            is String -> {
                val components = value.split(":")
...
@Suppress("UNCHECKED_CAST")
private fun Any.toDependencyVersion(): VersionDefinition? =
    when (this) {
...
private inline fun <reified T> Map<String, Any>.toTypedMap(): Map<String, T> =
    map {
        if (it.value !is T) {
