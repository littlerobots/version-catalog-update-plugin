...
class VersionCatalogWriter {
    fun write(
        ...
        writer: Writer,
        commentEntry: (HasVersion) -> Boolean = { false },
    ) {
    ...
    private fun formatPlugin(plugin: Plugin): String =
        when (plugin.version) {
            ...
            is VersionDefinition.Condition -> {
                StringBuilder("{ id = \"${plugin.id}\", version = { ")
                    .apply {
     ...
     */
    private fun formatLibrary(library: Library): String =
        when (library.version) {
            ...
            is VersionDefinition.Condition -> {
                StringBuilder("""{ module = "${library.module}", version = { """)
                    .apply {
    ...
    private fun formatVersion(versionDefinition: VersionDefinition): String =
        when (versionDefinition) {
