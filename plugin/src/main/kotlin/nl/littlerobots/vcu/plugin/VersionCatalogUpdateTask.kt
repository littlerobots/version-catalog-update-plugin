...
abstract class VersionCatalogUpdateTask : DefaultTask() {
    ...
    private fun printWarnings(result: DependencyResolverResult, currentCatalog: VersionCatalog) {
    ...
    private fun ensureSupportedSaxParser() {
        if ((
            GradleVersion.current() >= GradleVersion.version("7.6.3") && GradleVersion.current() <= GradleVersion.version(
                    "8.0"
                )
