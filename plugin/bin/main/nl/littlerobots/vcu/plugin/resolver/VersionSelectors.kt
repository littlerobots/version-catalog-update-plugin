    ...
    companion object {
        ...
        val LATEST =
            object : ModuleVersionSelector {
                override fun select(candidate: ModuleVersionCandidate): Boolean = true
            }
        ...
        val STABLE =
            object : ModuleVersionSelector {
                override fun select(candidate: ModuleVersionCandidate): Boolean = isStable(candidate.candidate.version)
            }
        ...
        val PREFER_STABLE =
            object : ModuleVersionSelector {
                override fun select(candidate: ModuleVersionCandidate): Boolean =
                    (!isStable(candidate.candidate.version) && !isStable(candidate.currentVersion)) ||
                        isStable(candidate.candidate.version)
            }
    }
}
