...
class VersionCatalogUpdatePlugin : Plugin<Project> {
    override fun apply(project: Project) {
        ...
        val defaultVersionCatalog =
            project.objects
                .newInstance(VersionCatalogConfig::class.java, "")
    ...
    private fun configureApplyTask(
        project: Project,
        versionCatalogConfig: VersionCatalogConfig,
    ) {
        project.tasks.register(
            "${VersionCatalogApplyUpdatesTask.TASK_NAME}${versionCatalogConfig.name.replaceFirstChar {
                if (it.isLowerCase()) {
                    it.titlecase(
    ...
    private fun configureFormatTask(
        project: Project,
        versionCatalogConfig: VersionCatalogConfig,
    ) {
        project.tasks.register(
            "$FORMAT_TASK_NAME${versionCatalogConfig.name.replaceFirstChar {
                if (it.isLowerCase()) {
                    it.titlecase(
                        java.util.Locale.getDefault(),
                    )
    ...
    private fun configureExperimentalUpdateTask(
        project: Project,
        versionCatalogConfig: VersionCatalogConfig,
    ) {
        ...
        val catalogUpdatesTask =
            project.tasks.register(
                "${UPDATE_TASK_NAME}${versionCatalogConfig.name.replaceFirstChar {
                    if (it.isLowerCase()) {
                        it.titlecase(
                            java.util.Locale.getDefault(),
                        )
        ...
        catalogUpdatesTask.configure { task ->
            ...
            val versionSelector =
                versionCatalogConfig.versionSelector
                    .orElse(extension.versionSelector)
