...
class DependencyResolverTest {
    ...
    fun `Resolves basic updates`() {
        ...
        val result =
            resolver.resolveFromCatalog(
                ...
                object : ModuleVersionSelector {
                    override fun select(candidate: ModuleVersionCandidate): Boolean = candidate.candidate.version == "1.3.1"
                },
            )
        ...
        assertEquals(
            "1.3.1",
            (
                result.versionCatalog.libraries.values
                    .first()
                    .version as? VersionDefinition.Simple
            )?.version,
        )
    }
    ...
    fun `Handles capability resolving errors for updates`() {
        ...
        val result =
            resolver.resolveFromCatalog(
                ...
                catalog,
                object : ModuleVersionSelector {
                    override fun select(candidate: ModuleVersionCandidate): Boolean = candidate.candidate.version == "1.6.0-beta03"
                },
            )
    ...
    private fun String.asStream(): ByteArrayInputStream = ByteArrayInputStream(toByteArray())
}
